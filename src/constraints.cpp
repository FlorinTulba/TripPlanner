  ...
  TimeConstraints::TimeConstraints(const time_period &leavePeriod_/* =
                                   ...
                                   const time_period &arrivePeriod_/* =
									                  time_period(
										                  nowUTC(),
										                  hours(366 * 24))*/) :
		  _leavePeriod(leavePeriod_.begin(),
                   leavePeriod_.end() + time_duration::unit()),
		  _arrivePeriod(arrivePeriod_.begin(),
                    arrivePeriod_.end() + time_duration::unit()) {
	  if(_leavePeriod.begin() > _arrivePeriod.begin() ||
	     _leavePeriod.last() > _arrivePeriod.last())
		  throw invalid_argument(string(__func__) +
                             " leavePeriod_ must begin before arrivePeriod_ "
                             "starts and must end before arrivePeriod_ ends");
  }
